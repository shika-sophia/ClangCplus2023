...
*         if ( 0 )    <=>  if (false) 
* 
*@subject ◆Should not compare both boolean values 〔C99YH p129, p391〕
*         because conditional expression 'x == TRUE' can be false as below [Example].
*         ・A if-statement recognize '0' as false and 'not 0' as true.
*           Conditional expression 'x == 0' can be false, though 'not 0' value should be true.
*         ・Generally, the code which verify to equal both boolean values, 
